...
module hessian_conv #(
  ...
  localparam OUT_SIZE = ROI_SIZE + KERNEL_SIZE - 1;
  localparam PAD_SIZE = (KERNEL_SIZE - 1) / 2;
  localparam BUF_WIDTH = ROI_SIZE + (MASK_SIZE - 1) * 2;  // WARN

  localparam HW_BITS = $clog2(OUT_SIZE);
  localparam BUF_WIDTH_BITS = $clog2(BUF_WIDTH);
  localparam FLAG_BITS = $clog2(KERNEL_SIZE);

  localparam FIRST_RIGHT_PAD_IDX = ROI_SIZE + (MASK_SIZE - 1);
