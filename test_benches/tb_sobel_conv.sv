...
module tb_sobel_conv;
  ...
  localparam OUT_NUM_PER_CYCLE = 2;

  localparam IN_WIDTH = 8;
  localparam KERNEL_DATA_WIDTH = 3;
  ...
  logic signed [OUT_WIDTH-1:0] sobel_dout[OUT_NUM_PER_CYCLE- 1:0];
  logic signed [OUT_WIDTH-1:0] max;
  logic sobel_conv_valid, ready;
  ...
  sobel_conv #(
      ...
      .OUT_NUM_PER_CYCLE(OUT_NUM_PER_CYCLE)
  ) sobel_conv_inst (
      .clk(clk),
      .rst_n(rst_n),
      .clk_en(clk_en),
  ...
  initial begin
    ...
    begin : Initialization_and_load
      ...
      $readmemh("D:\\Vivado\\Vivado_Projects\\Codes\\texts\\input_img.txt", in_img);
      $readmemb("D:\\Vivado\\Vivado_Projects\\Codes\\texts\\sobel_kernel.txt", kernel_vec);
    ...
    while (out_pixel_idx < ROI_AREA) begin : IO
      ...
      if (sobel_conv_valid) begin : write
        for (int i = 0; i < OUT_NUM_PER_CYCLE; i++) begin
          out_img[out_pixel_idx+i] = sobel_dout[i];
        end
        out_pixel_idx = out_pixel_idx + OUT_NUM_PER_CYCLE;
      end
    end

    begin : write_and_finish
      $writememh("D:\\Vivado\\Vivado_Projects\\Codes\\texts\\output_img.txt", out_img);
