...
module conv1_buf #(
  ...
  reg [DATA_BITS - 1:0] buffer  [0:WIDTH * FILTER_SIZE - 1];  // Low -> High, 和index对应
  reg [DATA_BITS - 1:0] buf_idx;
  reg [4:0] w_idx, h_idx;
  reg [2:0] buf_flag;  // 0 ~ 4
  reg state;

  // 每读取一个卷积核，buffer区的第一个像素就会被换掉
  always @(posedge clk) begin
    if (~rst_n) begin
      ...
      valid_out_buf <= 0;
      data_out_0 <= 12'bx;
      data_out_1 <= 12'bx; // Unknown会导致调试时难以区分是否正确, Don't Care会使得在综合时可以综合出效率更高的电路
      data_out_2 <= 12'bx;
      data_out_3 <= 12'bx;
      data_out_4 <= 12'bx;
      data_out_5 <= 12'bx;
      ...
      if (!state) begin
        ...
        if (w_idx == WIDTH - FILTER_SIZE + 1) begin
          valid_out_buf <= 1'b0;  // invalid area
